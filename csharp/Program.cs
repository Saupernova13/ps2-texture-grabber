...
var entry   = new GameDbService(log).Resolve(gameDb, cli.Query, cli.Interactive);
...
internal sealed class Args
{
    ...
    public string? FlareSolverrUrl  { get; private set; }
    ...
    public static Args Parse(string[] argv)
    {
        ...
        for (int i = 0; i < argv.Length; i++)
        {
            switch (argv[i].ToLowerInvariant())
            {
                ...
                case "--flaresolverr-url":       a.FlareSolverrUrl = Next(argv, ref i); break;
                case "--node-id":
